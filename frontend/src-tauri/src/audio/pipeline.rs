...
pub struct AudioPipeline {
    ...
    state: Arc<RecordingState>,
    // Dual VAD processors for separate mic/system transcription
    ...
    // Buffers for accumulating raw mic/system samples before VAD
    mic_vad_buffer: Vec<f32>,
    system_vad_buffer: Vec<f32>,
}
...
impl AudioPipeline {
    pub fn new(
        ...
        // Create dual VAD processors for separate mic/system transcription
        // This enables speaker identification by tracking which audio source produced each segment
        ...
        let vad_processor_mic = match ContinuousVadProcessor::new(sample_rate, redemption_time) {
            Ok(processor) => {
                info!("Mic VAD processor created for separate speaker identification");
                processor
            }
            Err(e) => {
        ...
        let vad_processor_system = match ContinuousVadProcessor::new(sample_rate, redemption_time) {
            Ok(processor) => {
                info!("System VAD processor created for separate speaker identification");
                processor
            }
            Err(e) => {
        ...
        loop {
            ...
            match tokio::time::timeout(
                ...
                Ok(Some(chunk)) => {
                    ...
                    // STEP 1: Accumulate raw audio for per-source VAD (speaker identification)
                    match &chunk.device_type {
                        DeviceType::Microphone => self.mic_vad_buffer.extend(&chunk.data),
                        DeviceType::System => self.system_vad_buffer.extend(&chunk.data),
                    }

                    // STEP 2: Add raw audio to ring buffer for mixing (recording path)
                        ...
                        if let Some((mic_window, sys_window)) = self.ring_buffer.extract_window() {
                            ...
                            if let Some(ref sender) = self.recording_sender_for_mixed {
                                let recording_chunk = AudioChunk {
                                    ...
                                    device_type: DeviceType::Microphone,  // Mixed audio for recording
                                };
                    ...
                    // STEP 4: Run separate VAD on mic audio for speaker-tagged transcription
                    if !self.mic_vad_buffer.is_empty() {
                        let mic_samples = std::mem::take(&mut self.mic_vad_buffer);
                        match self.vad_processor_mic.process_audio(&mic_samples) {
                            Ok(speech_segments) => {
                    ...
                    // STEP 5: Run separate VAD on system audio for speaker-tagged transcription
                    if !self.system_vad_buffer.is_empty() {
                        let sys_samples = std::mem::take(&mut self.system_vad_buffer);
                        match self.vad_processor_system.process_audio(&sys_samples) {
                            Ok(speech_segments) => {
                                for segment in speech_segments {
                                    let duration_ms = segment.end_timestamp_ms - segment.start_timestamp_ms;
                                    if segment.samples.len() >= 800 {
    ...
    fn flush_remaining_audio(&mut self) -> Result<()> {
        ...
        // Flush mic VAD processor
        match self.vad_processor_mic.flush() {
            Ok(final_segments) => {
        ...
        // Flush system VAD processor
        match self.vad_processor_system.flush() {
            Ok(final_segments) => {
                for segment in final_segments {
                    if segment.samples.len() >= 800 {
                        let transcription_chunk = AudioChunk {
