  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center h-full">
        <LoaderIcon className="w-6 h-6 animate-spin text-text-placeholder" />
      </div>
    );
  }
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center">
            <MessageSquare className="w-10 h-10 text-text-placeholder mb-3" />
            <h3 className="text-sm font-medium text-text-secondary mb-1">
              {meetingId ? 'Ask about this meeting' : 'Ask across meetings'}
            </h3>
            <p className="text-xs text-text-placeholder max-w-[250px]">
              {meetingId
          ...
          <>
            ...
            {isSending && (
              <div className="flex gap-2.5">
                <div className="w-7 h-7 rounded-full flex items-center justify-center shrink-0 bg-surface-tertiary">
                  <LoaderIcon className="w-3.5 h-3.5 animate-spin text-muted-foreground" />
                </div>
                <div className="px-3 py-2 rounded-lg bg-surface-tertiary text-sm text-muted-foreground rounded-bl-sm">
                  Thinking...
                </div>
              </div>
            )}
