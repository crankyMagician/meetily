  ...
  return (
    <div className={`flex items-center space-x-2 ${className}`}>
      ...
      <div className={`w-2 h-2 rounded-full ${
        isActive ? 'bg-green-400 animate-pulse' : 'bg-surface-hover'
      }`} title={`${deviceName} - ${isActive ? 'Active' : 'Inactive'}`} />

      {/* Level meter container */}
      <div className={`flex-1 ${sizes.container} relative`}>
        {/* Background */}
        <div className="w-full h-full bg-surface-hover rounded-sm overflow-hidden">
      ...
      <div className={`${sizes.text} text-text-secondary font-mono min-w-[3rem] text-right`}>
        {rmsPercent}%
      </div>
    </div>
  );
  ...
  return (
    <div className={`flex items-center space-x-1 ${className}`}>
      ...
      <div className={`w-1.5 h-1.5 rounded-full ${
        isActive ? 'bg-green-400' : 'bg-surface-hover'
      }`} />

      {/* Mini meter */}
      <div className="w-8 h-1.5 bg-surface-hover rounded-sm overflow-hidden">
        <div
