  ...
  return (
    <div 
      className={`group relative min-h-[24px] flex items-start rounded transition-all duration-150 ease-in-out
        ${isSelected ? 'bg-blue-50 dark:bg-blue-950 ring-1 ring-blue-200 shadow-sm' : 'hover:bg-surface-secondary'}`}
      ...
      <div className="relative flex-1 py-0.5 px-1">
        <textarea
          ...
          className={`
            ...
            transition-all duration-150 ease-in-out text-foreground
        ...
        {showCommands && (
          <div 
            ref={commandsRef}
            className="absolute left-0 top-full mt-1 w-64 bg-surface rounded-lg shadow-lg border border-border py-2 z-50
          ...
          >
            {filteredCommands.map((cmd, index) => (
              <button
                key={cmd.id}
                className={`
                  w-full text-left px-3 py-2 flex items-center space-x-3 hover:bg-surface-secondary
                  ${index === selectedCommandIndex ? 'bg-surface-secondary' : ''}
                `}
                onClick={() => handleCommandSelect(cmd)}
                onMouseEnter={() => setSelectedCommandIndex(index)}
              >
                <span className="flex-shrink-0 w-6 h-6 flex items-center justify-center bg-surface-tertiary rounded text-text-secondary">
                  {cmd.icon}
                </span>
